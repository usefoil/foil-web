...
Future Foil releases can update or flag web install copy, checksum, Homebrew cask state, and docs through one repeatable check.
...
- Authority: `approved`
- Proof type: `test`
- Completion proof: A repeatable command compares the latest GitHub release and Homebrew cask against website install copy/docs/checks, then passes when aligned or fails with actionable drift details.
- Goal oracle: A repeatable command compares the latest GitHub release and Homebrew cask against website install copy/docs/checks, then passes when aligned or fails with actionable drift details.
- Likely misfire: Only documenting today’s release while future release drift still requires manual memory.
- Blind spots considered:
  - Network-dependent checks need timeouts and graceful skips in CI.
  - Homebrew tap may redirect from legacy org URLs.
  - Release asset names and checksums must remain exact.
- Existing plan facts:
...
`A repeatable command compares the latest GitHub release and Homebrew cask against website install copy/docs/checks, then passes when aligned or fails with actionable drift details.`
...
```text
/goal Follow docs/goals/release-update-automation/goal.md.
```
